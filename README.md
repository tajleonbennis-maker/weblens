...
> AI 驱动的交互式网络空间测绘：公开网络（FOFA / 真身浏览）+ 封闭社交平台（微信 / 抖音 / TikTok）数据获取，一句话驱动，全程留痕，输出可分享的情报报告。

WebLens 是一个把「网络空间测绘」做成**对话式工作台**的平台：输入一个域名、IP 或直接说一句话任务，平台用真实浏览器（Lightpanda）打开目标、提取结构化情报、识别登录注册入口并与之交互，还能通过 FOFA 查询全网资产、通过 TikHub 读取微信公众号等封闭平台内容。
